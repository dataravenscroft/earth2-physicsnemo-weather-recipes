...
`data/era5/*.npy` — synthetic demo fixtures generated by
`scripts/create_synthetic_era5.py`. These are committed deliberately so
that `build_manifest.py` and `run_inference_demo.py` run immediately after
`git clone` without any data download step.
